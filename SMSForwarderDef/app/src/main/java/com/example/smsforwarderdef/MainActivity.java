package com.example.smsforwarderdef;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            insertDummyContactWrapperAndroid26();
        } else {
            insertDummyContactWrapperAndroid25();
        }
...
//    }
    ...
    private void insertDummyContactWrapperAndroid26() {
        int READ_SMS = checkSelfPermission(android.Manifest.permission.READ_SMS);
        int RECEIVE_SMS = checkSelfPermission(android.Manifest.permission.RECEIVE_SMS);
        int SEND_SMS = checkSelfPermission(android.Manifest.permission.SEND_SMS);
        int READ_PHONE_NUMBERS = checkSelfPermission(android.Manifest.permission.READ_PHONE_NUMBERS);
        int READ_PHONE_STATE = checkSelfPermission(android.Manifest.permission.READ_PHONE_STATE);
        int READ_CALL_LOG = checkSelfPermission(android.Manifest.permission.READ_CALL_LOG);
        int READ_CONTACTS = checkSelfPermission(android.Manifest.permission.READ_CONTACTS);


        if (RECEIVE_SMS != PackageManager.PERMISSION_GRANTED) {
            requestPermissions(new String[]{android.Manifest.permission.SEND_SMS, android.Manifest.permission.READ_SMS, android.Manifest.permission.RECEIVE_SMS, android.Manifest.permission.READ_PHONE_NUMBERS, android.Manifest.permission.READ_PHONE_STATE, android.Manifest.permission.READ_CALL_LOG, Manifest.permission.READ_CONTACTS},

                    REQUEST_CODE_ASK_PERMISSIONS);
        }
    }

    private void insertDummyContactWrapperAndroid25() {
        int READ_SMS = checkSelfPermission(android.Manifest.permission.READ_SMS);
        int RECEIVE_SMS = checkSelfPermission(android.Manifest.permission.RECEIVE_SMS);
        int SEND_SMS = checkSelfPermission(android.Manifest.permission.SEND_SMS);
        int READ_PHONE_STATE = checkSelfPermission(android.Manifest.permission.READ_PHONE_STATE);
        ...
        if (RECEIVE_SMS != PackageManager.PERMISSION_GRANTED) {
            requestPermissions(new String[]{android.Manifest.permission.SEND_SMS, android.Manifest.permission.READ_SMS, android.Manifest.permission.RECEIVE_SMS, android.Manifest.permission.READ_PHONE_STATE, android.Manifest.permission.READ_CALL_LOG, Manifest.permission.READ_CONTACTS},
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CODE_ASK_PERMISSIONS) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                // Permission Denied
                Toast.makeText(MainActivity.this, "Need grant permissions!", Toast.LENGTH_SHORT).show();
            }
        } else {
